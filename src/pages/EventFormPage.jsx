  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h2 className="text-xl font-semibold text-zinc-900 mb-1">{t('evtEventCreated')}</h2>
        <p className="text-sm text-zinc-500 mb-6">
          <span className="font-medium text-zinc-700">{success.name}</span> {t('evtHasBeenCreated')}
        </p>
        <div className="flex gap-3">
          <button
            onClick={handleCreateAnother}
            className="px-4 py-2 bg-zinc-100 text-zinc-700 rounded-md text-sm font-medium hover:bg-zinc-200 transition-colors"
          >
            {t('evtCreateAnother')}
          </button>
          <button
    ...
    <div className="max-w-xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-0.5">{t('evtFormSubtitle')}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        {/* Name */}
        <div>
          <label className="block text-xs font-medium text-zinc-700 mb-1.5">{t('evtNameLabel')} *</label>
          <input
            type="text"
            value={name}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-medium text-zinc-700 mb-1.5">{t('evtStartDate')} *</label>
            <input
              type="datetime-local"
              value={startDate}
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-700 mb-1.5">{t('evtEndDate')} *</label>
            <input
              type="datetime-local"
              value={endDate}
        ...
        {/* Location */}
        <div>
          <label className="block text-xs font-medium text-zinc-700 mb-1.5">{t('evtLocationLabel')}</label>
          <input
            type="text"
            value={location}
        ...
        {isEdit && (
          <div>
            <label className="block text-xs font-medium text-zinc-700 mb-1.5">{t('colStatus')}</label>
            <select
              value={status}
        ...
        <button
          ...
          className="w-full py-2.5 bg-zinc-900 text-white rounded-md text-sm font-medium hover:bg-zinc-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {submitting
