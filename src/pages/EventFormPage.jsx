  ...
  if (success) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h2 className="text-[18px] font-extrabold text-zinc-900 mb-1">{t('evtEventCreated')}</h2>
        <p className="text-[13px] text-zinc-500 mb-6">
          <span className="font-semibold">{success.name}</span> {t('evtHasBeenCreated')}
        </p>
        <div className="flex gap-3">
          <button
            onClick={handleCreateAnother}
            className="px-4 py-2.5 bg-zinc-100 text-zinc-700 rounded-xl text-[13px] font-semibold hover:bg-zinc-200 transition"
          >
            {t('evtCreateAnother')}
          </button>
          <button
            onClick={() => navigate(`/events/${success.eventId}`)}
            className="px-4 py-2.5 bg-zinc-900 text-white rounded-xl text-[13px] font-semibold hover:bg-zinc-800 transition"
          >
            {t('evtViewEvent')}
          </button>
        </div>
  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h2 className="text-[22px] font-extrabold text-zinc-900">
            {isEdit ? t('evtEditEvent') : t('evtNewEvent')}
          </h2>
          <p className="text-[13px] text-zinc-400 mt-0.5">{t('evtFormSubtitle')}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        {/* Name */}
        <div>
          <label className="block text-[12px] font-semibold text-zinc-700 mb-1.5">{t('evtNameLabel')} *</label>
          <input
            type="text"
            value={name}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-[12px] font-semibold text-zinc-700 mb-1.5">{t('evtStartDate')} *</label>
            <input
              type="datetime-local"
              value={startDate}
          ...
          <div>
            <label className="block text-[12px] font-semibold text-zinc-700 mb-1.5">{t('evtEndDate')} *</label>
            <input
              type="datetime-local"
              value={endDate}
        ...
        {/* Location */}
        <div>
          <label className="block text-[12px] font-semibold text-zinc-700 mb-1.5">{t('evtLocationLabel')}</label>
          <input
            type="text"
            value={location}
        ...
        {isEdit && (
          <div>
            <label className="block text-[12px] font-semibold text-zinc-700 mb-1.5">{t('colStatus')}</label>
            <select
              value={status}
        ...
        {/* Error */}
        {submitError && (
          <div className="px-4 py-3 bg-red-50 border border-red-100 rounded-xl text-[12px] text-red-700">
            {submitError}
          </div>
        )}
        ...
        <button
          ...
          className="w-full py-3 bg-zinc-900 text-white rounded-xl text-[13px] font-semibold hover:bg-zinc-800 transition disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {submitting
